...
def index_dataset(dataset_name, base_path, force=False):
    print('Indexing dataset {0}'.format(dataset_name))
    base_path = osp.join(base_path, dataset_name)
...
def write_links(dataset, ent_out, small_ent_out, max_ans_num, name, base_path):
    ...
    base_path = osp.join(base_path, dataset)
    with open(osp.join(base_path,f'{name}-queries.pkl'), 'wb') as f:
        pickle.dump(queries, f)
    with open(osp.join(base_path,f'{name}-tp-answers.pkl'), 'wb') as f:
        pickle.dump(tp_answers, f)
    with open(osp.join(base_path,f'{name}-fn-answers.pkl'), 'wb') as f:
        pickle.dump(fn_answers, f)
    with open(osp.join(base_path,f'{name}-fp-answers.pkl'), 'wb') as f:
...
def ground_queries(dataset, query_structure, ent_in, ent_out, small_ent_in, small_ent_out, gen_num, max_ans_num, query_name, mode, ent2id, rel2id, base_path):
    ...
    while num_sampled < gen_num:
        ...
        answer = random.sample(sorted(ent_in.keys()), 1)[0]
    ...
    with open(osp.join(base_path, f'{name_to_save}-queries.pkl'), 'wb') as f:
        pickle.dump(queries, f)
    with open(osp.join(base_path, f'{name_to_save}-fp-answers.pkl'), 'wb') as f:
        pickle.dump(fp_answers, f)
    with open(osp.join(base_path, f'{name_to_save}-fn-answers.pkl'), 'wb') as f:
        pickle.dump(fn_answers, f)
    with open(osp.join(base_path, f'{name_to_save}-tp-answers.pkl'), 'wb') as f:
        pickle.dump(tp_answers, f)
...
def generate_queries(dataset, query_structures, gen_num, max_ans_num, gen_train, gen_valid, gen_test, query_names, save_name, base_path):
    base_path = osp.join(base_path,dataset)
    ...
    #     if gen_train:
    #         write_links(dataset, train_ent_out, defaultdict(lambda: defaultdict(set)), max_ans_num, 'train-'+query_name, base_path)
    #     if gen_valid:
    #         write_links(dataset, valid_only_ent_out, train_ent_out, max_ans_num, 'valid-'+query_name, base_path)
    #     if gen_test:
    #         write_links(dataset, test_only_ent_out, valid_ent_out, max_ans_num, 'test-'+query_name, base_path)
    #     print ("link prediction created!")
    #     exit(-1)
    
    name_to_save = query_name
    set_logger(save_path=base_path, query_name=name_to_save, print_on_screen=True)
    ...
    if gen_train:
        train_queries, train_tp_answers, train_fp_answers, train_fn_answers = ground_queries(dataset, query_structure, 
            ...
            gen_num[0], max_ans_num, query_name, 'train', ent2id, rel2id, base_path)
    if gen_valid:
        valid_queries, valid_tp_answers, valid_fp_answers, valid_fn_answers = ground_queries(dataset, query_structure, 
            valid_ent_in, valid_ent_out, train_ent_in, train_ent_out, gen_num[1], max_ans_num, query_name, 'valid', ent2id, rel2id, base_path)
    if gen_test:
        test_queries, test_tp_answers, test_fp_answers, test_fn_answers = ground_queries(dataset, query_structure, 
            test_ent_in, test_ent_out, valid_ent_in, valid_ent_out, gen_num[2], max_ans_num, query_name, 'test', ent2id, rel2id, base_path)
...
@click.option('--base_path', default='/home/sagemaker-user/git/UltraQuery/query-datasets')
...
def main(dataset, seed, gen_train_num, gen_valid_num, gen_test_num, max_ans_num, reindex, gen_train, gen_valid, gen_test, gen_id, save_name, index_only, base_path):
    train_num_dict = {'FB15k': 273710, "FB15k-237": 149689, "NELL": 107982, "PrimeKG1":101}#6480398}
    valid_num_dict = {'FB15k': 8000, "FB15k-237": 5000, "NELL": 4000, "PrimeKG1":101}#810050}
    test_num_dict = {'FB15k': 8000, "FB15k-237": 5000, "NELL": 4000, "PrimeKG1":101}#810050}
    ...
    if index_only:
        index_dataset(dataset, base_path, reindex)
        exit(-1)
    ...
    generate_queries(
        dataset,
        query_structures[gen_id:gen_id+1],            # query_structures
        [gen_train_num, gen_valid_num, gen_test_num], # gen_num
        max_ans_num,
        gen_train,
        gen_valid,
        gen_test,
        query_names[gen_id:gen_id+1],                 # query_names
        save_name,
        base_path
    )
